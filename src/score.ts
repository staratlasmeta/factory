...
/**
 * Returns the SCORE Log base IDL for the SCORE program following as generated by Anchor with provided program ID appended to metadata.
 *
 * @param programId - Deployed program ID for the SCORE program
 * @returns - The base IDL object for score log
 */
 export function getScoreLogIDL(
  programId: web3.PublicKey,
): unknown {
  const _tmp = scoreLogBaseIdl;
  // _tmp['metadata']['address'] = programId.toBase58();
  return _tmp;
}
 ...
 */
 export async function decodeEvent(
  ...
  const provider = new Provider(connection, null, null);
  const idl = getScoreLogIDL(programId);
  const program = new Program(<Idl>idl, programId, provider);

  const decoded = program.coder.events.decode(event);
  return decoded;
}
